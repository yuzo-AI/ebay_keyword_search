...
#### 安定版（リトライ機能付き）
```bash
python extract_ebay_data_stable.py
```

#### 最高価格版
```bash
python extract_ebay_seller_hub_highest_price.py
```
...
- `seller_hub_config_型番_YYYYMMDD_HHMMSS.html`: 検索結果のHTMLダンプ
- 問題発生時の原因調査に使用
...
2. **「商品名がとれてない」**
   - ログイン後、検索窓に型番が入力されているか確認
   - デバッグHTMLファイルで検索結果を確認
